...
## Fitting quality gate (revision 2.2.0)

See [fitting quality review](fit-quality-review.md) for the rear scalp, selective accessory interaction, wrapped eyewear, and pigment corrections. Every hairstyle now uses `assets/source/scalp_foundation.py`, which samples the same asymmetric skull sections and angular layout as the head builder. Hairline design and lock styling remain separate from this shared fit surface. The envelope covers the widest currently authored head; a future head must pass the full rear/temple/crown matrix before joining this family. A matching face-family label alone does not qualify arbitrary imported skull geometry.
