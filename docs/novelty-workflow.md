...
Galaxy mounts on `root` and uses the existing `orbit` effect. It introduces no simulation object or physical gameplay advantage. Its authored annulus is now 0.825–0.99 m after full 5.4 m/s sprint geometry reached 0.787 m. The source `fit_orbit_miniature` helper scales each complete miniature uniformly about its own center and relocates its full horizontal footprint inside this annulus; hulls, decals and rings retain their proportions. This preserves the sub-1 m cosmetic bound while making room for the stride. [rebuild_orbit.py](../assets/source/rebuild_orbit.py) runs through interactive Blender in a dedicated scene, updates only this GLB/catalog record, and saves [orbital-accessory.blend](../assets/source/orbital-accessory.blend). The radial clearance check accounts for each miniature's full footprint and the moving legs, rather than comparing only object centers. The [Blender review of the revised annulus](evidence/novelty/orbit-sprint-annulus.png) confirms the UFO, ringed planet and comic star retain their silhouettes. Reduced motion freezes the orbit at its initial orientation.
