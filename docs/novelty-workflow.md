...
Galaxy mounts on `root` and uses the existing `orbit` effect. It introduces no simulation object or physical gameplay advantage. Its authored annulus is **1.02–1.185 m**, qualified against the corrected native locomotion. The earlier 0.825–0.99 m orbit cleared the previous compressed stride; the native clips now reach 0.967865 m in the ankle-height band. The new annulus leaves 52.1 mm measured clearance and explicitly expands the cosmetic outer bound from 1 m to 1.2 m.

The source `fit_orbit_miniature` helper fits the complete horizontal footprint of each miniature into the annulus. Keeping its width at 0.165 m preserves the UFO, Saturn and star's previous physical sizes and proportions; only their orbit radius changes. Decals, tilted rings and hulls move together. The fit is authored once, with no per-frame mesh scan, hairstyle-specific copy or stride-dependent shrinking. Bounding the full footprint keeps it clear at every rotation of the orbital assembly.

[rebuild_orbit.py](../assets/source/rebuild_orbit.py) runs through interactive Blender in a dedicated scene, updates only this GLB/catalog record, and saves [orbital-accessory.blend](../assets/source/orbital-accessory.blend). The [top annulus review](evidence/novelty/orbit-native-stride-annulus.png) and [oblique miniature review](evidence/novelty/orbit-native-stride-hero.png) render the actual rebuilt geometry. Their concentric guides show the measured leg envelope and qualified inner/outer limits; these review-only guides are not in the exported asset. Reduced motion still freezes the orbit at its initial orientation.

The [native-stride qualification](evidence/novelty/orbit-native-stride-clearance.json) samples 12,960 actual assembled poses: all three shoes, five weights, idle, walk, and full 5.4 m/s travel at sixteen headings, with 48 continuous frames per condition. It checks the entire deformed mesh in the orbit's height band, including diagonal clip blends. The regression retains a 25 mm minimum-clearance gate, fixed colors, above-floor placement and reduced-motion checks. The rebuilt effect remains **364 triangles / 25,272 bytes**; the current catalog is **2,190,020 bytes**, below its 2.2 MB limit. Native gait or shoe changes must requalify this envelope rather than silently letting an orbit overlap the stride.
...
Retain the 2.2 MB complete catalog gate (updated for [articulated footwear](articulated-footwear.md)) and each appearance's 14,000 source triangles, 1.5 MB and 12-part limits. Source estimates do not replace exported/fitted measurements, and successful local checks do not establish full-room phone performance.

| Asset            | Source triangles | GLB bytes |
| ---------------- | ---------------: | --------: |
| Quack Captain    |              584 |    20,416 |
| Starstruck Specs |              480 |    24,912 |
| Pocket Galaxy    |              364 |    25,272 |

The 39-model catalog totals **2,076,852 bytes**. All **60 avatar unit tests** pass, including 933,120 structural recipes, 486 scalp-coverage states and 180 complete head/hair/hat/glasses budget states. The maximum measured fitted assembly remains **13,900 source triangles**. Existing and synthetic wide/tall hairstyles have zero measured hat intersections under the original checks. The current orbit test samples all three shoes, three weights, idle, walk, forward/back/left/right full sprint and 36 continuous frames per velocity (1,944 poses). It also verifies reduced-motion freezing and fixed accessory colors. The catalog totals and suite counts in this section record the original novelty release; articulated footwear and reference locomotion have separate current qualification records.
