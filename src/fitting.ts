...
export function fitAssembly(
  ...
  for (const asset of assets) {
    ...
    if (!projections.has(provider.id))
      projections.set(
        provider.id,
    ...
    for (const mesh of owned.get(asset.id) ?? []) {
      ...
      for (let i = 0; i < position.count; i++) {
        ...
        if (depth !== undefined) {
          ...
          throw new AvatarError(
            "fit",
            `${asset.label} extends beyond ${provider.label}'s fitting surface`,
          );
        }
        vertices.push({ mesh, index: i, local: p, inverse, depth });
      }
    }
    if (
      !hits ||
    ...
    for (const { mesh, index, local, inverse, depth } of vertices) {
      const dz = fit.mode === "surface" ? depth! + fit.offset - local.z : shift;
      if (Math.abs(dz) > fit.maxDistance)
        throw new AvatarError(
          "fit",
